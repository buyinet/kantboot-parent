...
public interface SysDictI18nRepository extends JpaRepository<SysDictI18n, Long>
{
     ...
     */
    @Query("""
    FROM SysDictI18n dictI18n
    WHERE dictI18n.languageCode = :languageCode
        AND dictI18n.dictGroupCode = :dictGroupCode
        AND dictI18n.dictCode = :dictCode"""
    )
    List<SysDictI18n> findByLanguageCodeAndDictGroupCodeAndDictCode(String languageCode, String dictGroupCode, String dictCode);

    /**
     * 根据语言编码、分组编码查询、文字查询
     */
    @Query("""
    SELECT COUNT(dictI18n) FROM SysDictI18n dictI18n
    WHERE dictI18n.languageCode = :languageCode
        AND dictI18n.dictGroupCode = :dictGroupCode
        AND dictI18n.value = :value
""")
    Long countByLanguageCodeAndDictGroupCodeAndValue(String languageCode, String dictGroupCode, String value);
    ...
    @Modifying
    @Query(value = """
        insert into sys_dict_i18n (language_code, dict_code, dict_group_code, t_value)
        values (:#{#param.languageCode}, :#{#param.dictCode}, :#{#param.dictGroupCode}, :#{#param.value})
        """, nativeQuery = true)
    void insert(@Param("param") SysDictI18n param);
