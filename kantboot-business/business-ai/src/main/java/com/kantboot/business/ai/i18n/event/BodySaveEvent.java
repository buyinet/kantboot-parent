...
public class BodySaveEvent {
    ...
    public void onBusAiChatModelTypeSave(Long bodyId) {
        ...
        // 源语言
        String sourceLanguageCode = byId.getSourceLanguageCode();
        ...
        for (SysLanguage language : bySupport) {
            // 通过英文翻译成全部支持的语言
            String s = dictI18nService.translateText(sourceLanguageCode, language.getCode(), byId.getName());
    ...
    public void onBusAiChatModelSave(Long bodyId) {
        ...
        String sourceLanguageCode = model.getSourceLanguageCode();
        if(sourceLanguageCode == null){
            sourceLanguageCode = "en";
        }
        ...
        for (SysLanguage language : bySupport) {
            ...
            if(model.getName()!=null&&!("".equals(model.getName()))){
                String name = dictI18nService.translateText(sourceLanguageCode, language.getCode(), model.getName());
                modelI18n.setName(name);
            }
            if(model.getDescription()!=null&&!("".equals(model.getDescription()))){
                String description = dictI18nService.translateText(sourceLanguageCode, language.getCode(), model.getDescription());
                modelI18n.setDescription(description);
            }
            if(model.getIntroduction()!=null&&!("".equals(model.getIntroduction()))){
                String introduction = dictI18nService.translateText(sourceLanguageCode, language.getCode(), model.getIntroduction());
            ...
            for (BusAiChatModelLabel label : model.getLabels()) {
                ...
                String text = dictI18nService.translateText(sourceLanguageCode, language.getCode(), label.getText());
